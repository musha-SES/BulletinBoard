...
require('../library.php');
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        if($form['name'] === ''){ //空文字チェック
            $error['name']	= 'blank'; //errorメッセージ格納
        }

        /*-----メールチェック-----*/
        $form['email'] = filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL);
        if ($form['email'] === '') { //空文字チェック
            $error['email'] = 'blank';  
        ...
        } else if (strlen($form['password']) < 4){ //strlenでpasswordの文字数をカウント
            $error['password'] = 'length';
        }

        /*-----画像のチェック-----*/
        $image = $_FILES['image'];
        if($image['name'] !== '' && $image['error'] === 0){ // 空チェック
            $type = mime_content_type($image['tmp_name']); //ファイルチェック
            if($type !== 'image/png' && $type !== 'image/jpeg'){ //jpeg/png以外ははじく
                $error['image'] = 'type';
            }
        }

        if(empty($error)) { //エラーが無ければtrue
            $_SESSION['form'] = $form;

            /*-----画像のアップロード-----*/
            if ($image['name'] !== ''){ //空でなければtrue
